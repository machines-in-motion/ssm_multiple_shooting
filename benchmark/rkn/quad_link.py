...
len_filt = 50
train_obs = train_data.reshape((train_data.shape[0], train_data.shape[1], 48, 48, 1))
...
test_obs = test_data.reshape((test_data.shape[0], test_data.shape[1], 48, 48, 1))
...
# Build Model
rkn = QuadLinkImageImputationRKN(
    observation_shape=train_obs.shape[-3:],
    latent_observation_dim=100,
...
def evaluate(pred, GT):
    ...
    for j in range(len(pred)):
        ...
        for i in range(pred.shape[1] - len_filt):
            ...
            mse_score += np.linalg.norm(tar - hat) ** 2 / (48 * 48)
            bce_score += -np.mean(
