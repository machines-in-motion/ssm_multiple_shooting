...
data_name = "4-Pendulum"
data_path = os.path.join(main_folder, "ssm_ms/data/datasets")
train_name = os.path.join(data_path, f"4_link_image_train.npy")
test_name = os.path.join(data_path, f"4_link_image_test.npy")
...
len_filt = 50
train_obs = train_data.reshape(
    (train_data.shape[0], train_data.shape[1], 1, im_size, im_size)
)
...
test_obs = test_data.reshape(
    (test_data.shape[0], test_data.shape[1], 1, im_size, im_size)
)
