...
if arg.ll:
    ...
    class transition_model(nn.Module):
        def __init__(self, dim_x, ne=64):
            super(transition_model, self).__init__()
            self.dim_x = dim_x
            self.ne = ne
            self.alpha = nn.Sequential(
                nn.Linear(dim_x, 2048),
                nn.ReLU(),
                nn.Linear(2048, self.ne),
                nn.Softmax(dim=1),
            )
...
class observation_model(nn.Module):
    def __init__(self, dim_x, dim_y, ne=16):
        ...
        self.conv_layers = nn.Sequential(
            nn.ConvTranspose2d(ne, 16, (5, 5), stride=(3, 3)),
            nn.LayerNorm((16, 11, 11)),
            nn.ReLU(),
            nn.ConvTranspose2d(16, 16, (3, 3), stride=(2, 2)),
            nn.LayerNorm((16, 23, 23)),
            nn.ReLU(),
            nn.ConvTranspose2d(16, 12, (3, 3), stride=(2, 2)),
            nn.LayerNorm((12, 47, 47)),
            nn.ReLU(),
...
print("----------    Training data     --------------")
Train_set, Test_set = utils.load_pendulum_data(4)
...
normalize = False
model = SSM(
    f_net, g_net, Train_set, nodes, dim_x, ms, lr, bs, alpha, normalize, "4-pendulum"
)

N_epoch = 1000
model.train(N_epoch)
model.save("4-Pendulum", log_name)
...
rnn_tools.create_gif(obs_pred, obs_GT, 50, "4-pendulum_train", True)
...
class RNN(nn.Module):
    def __init__(self, state_dim, latent_dim):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(1, 16, 3),
            nn.LayerNorm((16, 46, 46)),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=(2, 2)),
            nn.Conv2d(16, 16, 3),
            nn.LayerNorm((16, 21, 21)),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=(2, 2)),
            nn.Conv2d(16, 16, 3),
            nn.LayerNorm((16, 8, 8)),
